...
4. **Bring it up** (Traefik stays off; ports 8080 + 8081 are published):
   ```bash
   docker compose up --build
   ```
   Wait for `hc-seed` to finish with its summary, then `hc-mcp-server` to report healthy.

5. **Make the ports public:** in the **Ports** tab, find **8080** (mcp-server) and **8081**
...
6. **Connect Claude Code Desktop** to the MCP server:
   ```
   https://<your-codespace>-8080.app.github.dev/mcp
   ```
   (Same value as `PUBLIC_MCP_URL` + `/mcp` — copy 8080's URL from the Ports tab.)

7. Trigger a tool. The OAuth flow opens your system browser at the Keycloak URL
